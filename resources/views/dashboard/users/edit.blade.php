        ...
        <div id="myTabContent" class="tab-content">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select class="form-control" name="user_role">
                                <option value="user" {{$user->user_role == 'user' ? 'selected' : '' }}>{{__('site.user')}}</option>
                                <option value="company" {{$user->user_role == 'company' ? 'selected' : '' }}>{{__('site.company')}}</option>
                                <option value="supervisor" {{$user->user_role == 'supervisor' ? 'selected' : '' }}>{{__('site.supervisor')}}</option>
                                <option value="mediator" {{$user->user_role == 'mediator' ? 'selected' : '' }}>{{__('site.mediator')}}</option>
                                <option value="seller_tashalih" {{$user->user_role == 'seller_tashalih' ? 'selected' : '' }}>{{__('site.seller_tashalih')}}</option>
                                <option value="seller_manufacturing" {{$user->user_role == 'seller_manufacturing' ? 'selected' : '' }}>{{__('site.seller_manufacturing')}}</option>
                                <option value="admin" {{$user->user_role == 'admin' ? 'selected' : '' }}>{{__('site.admin')}}</option>
                            </select>
                            ...
                            <select class="form-control" name="rule_id">
                                <option value="0">@lang('site.choose_rule')</option>
                                  ...
                                  @foreach($rules as $rule)
                                      ...
                                      {{ $user->rule_id == $rule->id ? 'selected' : '' }}>{{ $rule->name }}</option>
                                  @endforeach
            ...
            <div role="tabpanel" class="tab-pane fade" id="tab_content2" aria-labelledby="profile-tab">
                ...
                <table class="data table table-striped no-margin">
                    ...
                    <tbody>
                        @foreach ($myReqs as $k=>$myReq)
                            <tr>
                                <td> {{ $k+1 }} </td>                                
                                <td> <a href="{{ route('admin.request.show',$myReq->id) }}"> AZ-{{ $myReq->id }} </a> </td>
                                <td> {{ __('site.'.$myReq->request_type) }} </td>
                                <td>
                                     {{ $myReq->brand ? $myReq->brand['name'] : '' }} - 
                                     {{ $myReq->model ? $myReq->model['name'] : '' }} - 
                                     {{ $myReq->year }} - 
                                </td>

                                <td> {{  $myReq->pieceAlternaive->piece ? 
                                        $myReq->pieceAlternaive->piece['name']  : '' }} 
                                </td>
                                <td> {{ $myReq->city ? $myReq->city['name'] : '' }} </td>
                                <td> {{ $myReq->accept_offer == 1 ? __('site.yes') : __('site.no') }} </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
