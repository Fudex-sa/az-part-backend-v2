    ...
    @csrf
        ...
        @foreach ($cols as $col)
            @if($col != 'id' & $col != 'created_at' & $col != 'updated_at' & $col != 'verification_code' 
                ...
                & $col != 'country_id' & $col != 'remaining_stores')
                ...
                <div class="form-group">
                    ...
                    @else <div class="col-md-6 col-sm-6 col-xs-12"> @endif
                        ...
                        @elseif($col == 'address')
                        <input type="url" name="{{ $col }}" class="form-control" value="{{ $item->$col }}">                                
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select name="country_id" id="country_id" class="form-control">
                    ...
                    @foreach (countries() as $country)
                        <option value="{{ $country->id }}" {{ $item->country_id == $country->id ? 'selected' : '' }}> {{ $country['name_'.my_lang()] }} </option>
                    @endforeach
                </select>
            </div>
        </div>
